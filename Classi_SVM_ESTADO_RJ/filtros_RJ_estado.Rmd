...
```{r}
amostra = BaseBR2015 %>% filter (UFCENSO == 33)
```
...
# trabalha 
df = data.frame(amostra$VB01011)
colnames(df) = c("trabalha")

df = data.frame(amostra$VB01026)
colnames(df) = c("pos_estudos")

df = data.frame(amostra$VB13005)
colnames(df) = c("saude")

df = data.frame(amostra$VB13005)
colnames(df) = c("engravidou")
...
write.csv(df, "Testes_ML_classi_RJESTADO.csv", row.names = FALSE)
```
